...
from bettina.modeling.miller94.update_clean import image_dir
from bettina.modeling.miller94.update_clean.tools import tools
...
def plot_temporal_behaviour(st,**kwargs):
	...
	RF_t0,_ = get_RF(st[t0,...],DA=DA)
	RF_t1,_ = get_RF(st[t1,...],DA=DA)
	RF_t2,_ = get_RF(st[t2,...],DA=DA)
...
if __name__=="__main__":
	from bettina.modeling.miller94.update_clean import load_hdf5

	version = 28
	# params = tools.print_used_simulation_params()
	...
	try:
		...
		rA = 6.5
	DA = int(rA * 2)
	print("rI,rC",data["rI"][()]/rA,data["rC"][()]/rA)
	fig = plot_RF(s.reshape(N,N,N,N,2),DA=DA)
	fig.suptitle("rC={:.2f}, rI={:.2f}, rA={:.2f}".format(data["rC"][()]/rA,data["rI"][()]/rA,rA))
	pp = PdfPages(image_dir + "output_final_v{}.pdf".format(version))
	pp.savefig(fig,dpi=300,bbox_inches='tight')
	plt.close()
	pp.close()